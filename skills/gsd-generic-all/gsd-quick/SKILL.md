---
name: gsd:quick
description: [通用版] "Execute a quick task with GSD guarantees (atomic commits, state tracking) but skip optional agents"
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  gsd-sdk-gen query frontmatter.get .planning/quick/{dir}/SUMMARY.md status
  ```
...
Execute the quick workflow from @.planning/workflows/quick.md end-to-end.
