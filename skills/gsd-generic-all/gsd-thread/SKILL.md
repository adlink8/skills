---
name: gsd:thread
description: [通用版] "Manage persistent context threads for cross-session work"
...
- Read frontmatter `status` field via:
  ```bash
  gsd-sdk-gen query frontmatter.get .planning/threads/{file} status
  ```
...
2. Update the thread file's frontmatter `status` field to `resolved` and `updated` to today's ISO date:
   ```bash
   gsd-sdk-gen query frontmatter.set .planning/threads/{SLUG}.md status resolved
   gsd-sdk-gen query frontmatter.set .planning/threads/{SLUG}.md updated YYYY-MM-DD
   ```

3. Commit:
   ```bash
   gsd-sdk-gen query commit "docs: resolve thread — {SLUG}" ".planning/threads/{SLUG}.md"
   ```
...
```bash
gsd-sdk-gen query frontmatter.set .planning/threads/{SLUG}.md status in_progress
gsd-sdk-gen query frontmatter.set .planning/threads/{SLUG}.md updated YYYY-MM-DD
```
...
5. Commit:
   ```bash
   gsd-sdk-gen query commit "docs: create thread — ${ARGUMENTS}" ".planning/threads/${SLUG}.md"
   ```
...
- Status fields read via gsd-sdk-gen query frontmatter.get — never eval'd or shell-expanded
- The generate-slug call for new threads runs through gsd-sdk-gen query (or gsd-tools-gen) which sanitizes input — keep that pattern
</security_notes>
