---
name: gsd:graphify
description: [通用版] "Build, query, and inspect the project knowledge graph in .planning/graphs/"
...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-sdk-gen query`. Use `gsd-tools-gen graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-sdk-gen query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  gsd-tools-gen config-set graphify.enabled true
...
```bash
gsd-tools-gen graphify query <term>
```
...
```
PREFLIGHT=$(node "gsd-tools-gen" graphify build)
```
...
Project root: ${CWD}
gsd-tools-gen path: gsd-tools-gen
