---
name: gsd:debug
description: [通用版] "Systematic debugging with persistent state across context resets"
...
```bash
INIT=$(gsd-sdk-gen query state.load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Extract `commit_docs` from init JSON. Resolve debugger model:
```bash
debugger_model=$(gsd-sdk-gen query resolve-model gsd-debugger 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Read TDD mode from config:
```bash
TDD_MODE=$(gsd-sdk-gen query config-get workflow.tdd_mode 2>/dev/null | jq -r 'if type == "boolean" then tostring else . end' 2>/dev/null || echo "false")
```
